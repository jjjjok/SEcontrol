...
public class PageDao {
    ...
    //后台搜索用户分页
    public static int[] userNamePage(int count,int name){
        int result[]={0,1};
        Connection conn= DBUtil.getConnection();
        PreparedStatement ps = null;
        ResultSet rs =null;
        try {
            String sql="select count(*) from userinf where user_id=?";
            ps=conn.prepareStatement(sql);
            ps.setInt(1,name);
            rs=ps.executeQuery();
            while(rs.next()){
    ...
    //全部物品
    public static int[] itemByFlPage(int count,int fl){
        int result[]={0,1};
        Connection conn= DBUtil.getConnection();
        PreparedStatement ps = null;
        ResultSet rs =null;
        try {
            String sql="select count(*) from item where item_check=1 and item_id in(select item_id from link where fl_id=?)";
            ps=conn.prepareStatement(sql);
            ps.setInt(1,fl);
            rs=ps.executeQuery();
            while(rs.next()){
    ...
    public static int[] orderManagerPage(int count){
        ...
        ResultSet rs =null;
        try {
            String sql="select count(*) from orderinf where order_state is not null";
            ps=conn.prepareStatement(sql);
            rs=ps.executeQuery();
            while(rs.next()){
                result[0] = rs.getInt(1);
